...
read_hilda_strip_first_letter_add_column_id <- 
  function(filename){  
    ...
    temp %<>% mutate(tot_inc_percentile_contemporaneous = as.numeric(factor(cut(income,
                                                                                ...
                                                                                # entiles.
                                                                                breaks = tmp.svy.quantiles[c(1,7:11)],
                                                                                include.lowest = TRUE))),
                     top_decile = tot_inc_percentile_contemporaneous == 5,
                     second_dec = tot_inc_percentile_contemporaneous == 4,
...
all_hilda %>% 
  ...
  #select(xwaveid, top_decile, top_quintile, inc_quantile) %>%
  spread(inc_quantile, top_decile) %>%
  group_by(xwaveid) %>%
  summarise(Q1 = sum(Q1, na.rm=TRUE),
            Q2 = sum(Q2, na.rm=TRUE),
            Q3 = sum(Q3, na.rm=TRUE),
            Q4 = sum(Q4, na.rm=TRUE),
            Q5 = sum(Q5, na.rm=TRUE)) %>%
  select(contains("Q")) %>% 
  colMeans
@
