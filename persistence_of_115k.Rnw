...
decile_presence_by_top_decile_longevity %>%
  gather(decile, presence, -sum_topdecile) %>%
...
marimekko.data <- 
  ...
  group_by(decile) %>%
  mutate(xmin = lag(cumsum(N), default = 0.0),
         xmax = lead(xmin, default = Inf),
         xmax.b = ifelse(is.finite(xmax), xmax, max(cumsum_n))) %>%
  ungroup %>%
  group_by(sum_topdecile) %>%
  mutate(ymin = lag(cumsum(presence), default = 0),
         ymax = lead(ymin, default = 1)) %>%
  mutate(text = ifelse(xmin == min(xmin),
                       as.character(decile), 
                       NA_character_))

marimekko.data %>%
  grplot(aes(xmin = xmin, xmax = xmax,
             ymin = ymin, ymax = ymax,
             fill = decile)) + 
  geom_rect() + 
  geom_text(aes(x = 0.5 * (xmin + xmax),
                y = 0.5 * (ymin + ymax),
                label = ifelse(as.character(text) == "QX", "lower 50%", text),
                color = as.numeric(decile) <= 2),
            size = 7.14) + 
  scale_color_manual(values = c("black", "white")) + 
  scale_x_continuous("Years in top decile",
                     expand = list(c(0,0.075), c(0,0)),
                     breaks = unique((marimekko.data$xmin + marimekko.data$xmax.b)/2),
                     labels = 1:13) + 
