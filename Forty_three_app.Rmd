    ...
    sliderValues <- reactive({
      ...
      if (input$TargetFY != "2015-16"){
        fread("Just_Taxable_Income.csv") %>%
          mutate(Taxable_Income = as.integer(wage_inflator(Taxable_Income,
                                                           from_fy = "2012-13", 
                                                           to_fy = input$TargetFY))) %>%
          mutate(diff = income_tax(Taxable_Income) - new_income_tax(Taxable_Income),
                 ...
                 {-1 * sum(diff)* 50 * lf_inflator(to_date = as.character(fy2date(input$TargetFY)))} %>%
          grattan_dollar(.) ->
          the_amount_remaining
      } else {
        if (input$TargetFY == "2015-16"){
          fread("Just_Taxable_Income.csv") %>%
            mutate(Taxable_Income = wage_inflator(Taxable_Income,
                                                  from_fy = "2012-13", 
                                                  to_fy = "2014-15") * wage_inflator(from_fy = "2013-14",
                                                                                     to_fy = "2014-15")) %>%
            mutate(diff = income_tax(Taxable_Income) - new_income_tax(Taxable_Income),
                   diff = cpi_inflator(diff, 
                                       from_fy = "2012-13", 
