...
shinyApp( options = list(height = 1024),
  ...
  server = function(input, output) {
    sliderValues <- reactive({
      ...
      if (input$TargetFY != "2015-16"){
        cpi.inflator <- cpi_inflator(from_fy = "2012-13", to_fy = input$TargetFY)
        cpi.inflator = 1
        wage.inflator <- wage_inflator(from_fy = "2012-13", to_fy = input$TargetFY)
        lf.inflator <- lf_inflator(to_date = as.character(fy2date(input$TargetFY)))
        
        temp <- 
          fread("Just_Taxable_Income.csv") %>%
          mutate(Taxable_Income = as.integer(wage.inflator * Taxable_Income)) %>%
          mutate(tax_payable = income_tax(Taxable_Income),
                 new_tax_payable = new_income_tax(Taxable_Income)) %>%
          mutate(diff = tax_payable - new_tax_payable,
                 diff = diff * cpi.inflator)
        
        temp %$% 
        {-1 * sum(diff)* 50 * lf.inflator} %>%
          grattan_dollar(.) ->
          the_amount_remaining
        
        total_revenue <-
            temp %$%
            sum(tax_payable) * 50 * lf.inflator
          
        total_revenue <- grattan_dollar(total_revenue)
        
        new_revenue <- 
          temp %$%
          sum(tax_payable) * 50 * lf.inflator
        
        new_revenue <- grattan_dollar(new_revenue)
          
      } else {
        if (input$TargetFY == "2015-16"){
          cpi.inflator <- cpi_inflator(from_fy = "2012-13",                                         
                                       to_fy = "2014-15") * cpi_inflator(from_fy = "2013-14",
                                                                         to_fy = "2014-15")
          cpi.inflator = 1 
          wage.inflator <- wage_inflator(from_fy = "2012-13", 
                                         to_fy = "2014-15") * wage_inflator(from_fy = "2013-14",
                                                                            to_fy = "2014-15")
          lf.inflator <- lf_inflator(from_date = "2012-06-30", 
                                     to_date = "2015-06-30")
          ...
          new_revenue <- 
            temp %$%
            sum(new_tax_payable) * 50 * lf.inflator
      ...
      # Changing total income to taxable income. We assume a linear relationship and
      # a persistent one across years. 
      ...
      temp %>%
        #group_by(bracket = cut(Taxable_Income, breaks = c(input$bracket_1, input$bracket_2, input$bracket_3, input$bracket_4))) %>%
        summarise(average_tax_rate = mean(ifelse(Taxable_Income < 1, 0,
                                            income_tax(Taxable_Income, fy.year = input$TargetFY)/Taxable_Income
                                            )),
                  average_tax_rate_new = mean(ifelse(Taxable_Income < 1, 0,
                                                new_income_tax(Taxable_Income)/Taxable_Income))) ->
        avg_tax_rates
      ...
      data_frame(
        ...
        Value = as.character(c(input$rate_1, 
                               ...
                               avg_tax_rates$average_tax_rate,
                               avg_tax_rates$average_tax_rate_new
                               ))
        ) 
      }) 
