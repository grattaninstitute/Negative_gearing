...
fread(sample_file_of(2013)) %>% 
  ...
  annotate("segment", 
           ...
           xend = 10, yend = 0.30,
           color = '#DADADA') + 
  geom_dl(method = list("last.points", fontface = "bold", dl.trans(x=x+0.1), cex = 1.2)) + 
  scale_x_continuous(expand = c(0,0), breaks = 1:10) + annotate("blank", x = 13, y = 0.1) + theme(axis.line.x = element_blank()) + annotate("segment", x = 1, xend = 10, y = 0, yend = 0) + 
