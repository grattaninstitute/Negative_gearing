...
education_by_incomeQ <- 
  ...
  group_by(Quintile) %>%
  summarise(education.exp = sum(amount)) %>%
  ungroup %>%
  mutate(education.exp.prop = education.exp / sum(education.exp)) %>%
  data.table %>%
  setkey(Quintile)
...
chessboard_Q5 <- 
  ...
  mutate(est.education.spend = education.exp.y * education.exp.prop.x)
```
