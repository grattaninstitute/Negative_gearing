...
sih11 %>%
  ...
  coord_cartesian(xlim = c(-250,10e3),
                  ylim = c(-2,2))
```
...
perfect_and_imperfect_comp_adequacy %>%
  mutate(alpha = ifelse(comp_adequacy == 1, 1, 1),
         size  = ifelse(comp_adequacy == 1, 2.5, 1.25),
         linetype = ifelse(comp_adequacy == 1, "A", "B")) %>%
  ...
  geom_line() + 
  theme(axis.title.y = element_text(angle = 90)) + 
  scale_x_continuous("Percentage of total GST revenue required", label=scales::latex_percent) + 
  scale_y_continuous("Percentage of bottom quintile compensated by 25\\%, 50\\%, ... 175\\%\n", label=scales::latex_percent) + 
  scale_alpha_identity() + 
  scale_size_identity() +
...
hes10hh %>%
  ...
  summarise(
    percentage_compensated = weighted.mean(comp_adequacy * Weekly_household_GST_on_all_goods_and_services_FIS/2 < revenue_transfered,
                                           w = Weight_HH_HES)) %>%
  grplot(aes(x = comp_adequacy, y = percentage_compensated)) + 
