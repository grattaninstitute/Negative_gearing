...
bottom.quintile.not.compensated <- 
  ...
  mutate(not_compensated = new_welfare + tax_diff_weekly < Weekly_household_GST_on_all_goods_and_services_FIS/2,
         has_welfare = factor(Current_weekly_HH_income_from_government_pensions_and_allowances > 0),
...
party.cracks <- 
  party::ctree(factor(not_compensated) ~ Year_purchased_dwelling_HH_ranged_factor.ordered + Prev_fin_year_HH_disposable_income + Number_of_dependent_children_aged_under_years_in_household + age_group + Number_of_unemployed_persons_in_the_household + Net_wealth_of_household + has_welfare, 
               controls = ctree_control(minbucket = 150e3),
...
<<BottomQuintileFallingThroughCracks2, eval=FALSE>>=
