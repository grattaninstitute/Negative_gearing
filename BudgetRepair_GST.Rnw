...
new_0910_tax_rates = c(0, 0.13, 0.29, 0.38, 0.45)
...
\chapter{A targeted compensation package}
...
This will be difficult for governments that have been accustomed to being able to buy reform. The GST and the Carbon Pollution Reduction Scheme were both accompanied by a package of generous tax cuts and increases to welfare benefits (Box 2). The history of responding to demands that there be `no losers' has changed the political economy of reform. Politicians have arguably become less adept at prosecuting the case for difficult changes.\footcite[p.~80]{Megalogenis2010} 
...
But without revenue positive reforms, the budget pressures we outlined in our Fiscal Challenges paper will only continue to deteriorate. 

Our proposed package balances the need for fiscal consolidation with fairness and efficiency. It overcompensates the bottom 20 per cent of the income distribution on average, mainly through higher welfare payments. So most of the poorest Australians will be no worse off, and the majority better off, in terms of their real purchasing power. Modest tax cuts focussed on the low and middle thresholds help maintain the incentives for work participation and ensure that most low and middle income earners are also better off. Around 40 per cent of the additional revenue would be left over. This might:
\begin{itemize}
\item provide additional  revenue for state governments to reduce the hospital funding ``gap'';
\item provide additional revenue for the Commonwealth government to reduce its substantial deficit;
\item provide substitute revenue for further tax cuts, over and above the tax cuts focussed on the low and middle thresholds provided as part of the compensation package. 
\end{itemize}
...
\section{Higher welfare payments}\label{sec:higherWelfarePayments}
Governments can mitigate the effects of GST changes on lower income households -- indeed governments can choose to overcompensate -- through the welfare system.

Households at the bottom 20 per cent of the income distribution receive almost of all of their income via government payments -- including pensions, unemployment benefits and family allowances. The next poorest 20 per cent of households receive almost half of their incomes from these sources. Unsurprisingly, those further up the income distribution generate most of their income through employment, investments and super. These households receive little from government benefits (\Vref{fig:Sources_of_income_by_quintile}). 
...
welfare.by.income.quintile <- 
  ...
  select(Unique_household_number, Gross_income_quintile, 
         Weight_Person_HES, 
         Current_weekly_amount_of_Commonwealth_Rent_Assistance, 
         Current_weekly_income_from_age_pension, 
         Current_weekly_income_from_wife_pension, 
         Current_weekly_income_from_carer_payment, 
         Current_weekly_income_from_carer_allowance, 
         Current_weekly_income_from_special_benefit, 
         Current_weekly_income_from_widow_allowance, 
         Current_weekly_income_from_youth_allowance, 
         Current_weekly_income_from_Austudy_Abstudy, 
         Current_weekly_income_from_parenting_payment, 
         Current_weekly_income_from_Baby_Bonus_payment, 
         Current_weekly_income_from_partner_allowance, 
         Current_weekly_income_from_senior_supplement, 
         Current_weekly_income_from_newstart_allowance, 
         Current_weekly_income_from_pension_supplement, 
         Current_weekly_income_from_sickness_allowance, 
         Current_weekly_income_from_service_pension_DVA, 
         Current_weekly_income_from_utilities_allowance, 
         Current_weekly_income_from_disability_pension_DVA, 
         Current_weekly_income_from_war_widows_pension_DVA, 
         Current_weekly_income_from_disability_support_pension, 
         Current_weekly_income_from_family_tax_benefits_modelled, 
...
welfare.by.income.quintile %>%
  ...
  mutate(total = 52 * total/1e9) %>%
  mutate(text = ifelse(Gross_income_quintile == 2 & total > 1, gsub("_", " ", Allowance, fixed = TRUE), NA_character_)) %>%
  mutate(text = gsub("^([a-z]+)\\s([a-z]+)\\s(.*)$", "\\1 \\2\n\\3", text)) %>%
  group_by(Gross_income_quintile) %>%
  arrange(desc(total)) %>%
  ...
  geom_text(aes(label = text, y = text.y), vjust = 1.1, fontface = "bold", size = 5.5) + 
  scale_fill_manual(values = c(rep(RColorBrewer::brewer.pal(11, name="Spectral"), 2), "black")) + 
  scale_y_continuous(label = grattan_dollar, expand = c(0,0), limits = c(0,30)) + 
...
\makebox[\linewidth][r]{\includegraphics[width=1.1\columnwidth]{figure_BudgetRepair_GST/Welfare_by_household_disposable_income-1}}
...
welfare.by.income.quintile_total <-
  welfare.by.income.quintile %>%
  group_by(gross_income_quintile = Gross_income_quintile) %>%
...
\Sexpr{
paste0(
round(ceiling(100 * dplyr::filter(hes_with_gst_compensation, gross_income_quintile == 1, threshold == ">125%")$comp_adequacy)
, -1L),
"\\%")
}% 
...
\section{Income tax cuts}
Modest income tax cuts should be part of any GST reform package. 

Appropriately targeted income tax cuts will help to moderate the effect of a higher GST on work incentives. By increases prices of many good and services, a higher or broader GST, reduces the real purchasing power of take home pay. But tax cuts mean the size of workers' pay packets is larger. 
...
But if the purpose of GST reform is to help state governments fund rising healthcare costs, there is a limit to the money available to fund income tax reductions. Given our proposed welfare package, spending any more than 30 per cent of the revenue on tax cuts will not leave state governments with enough funds to make a meaningful dent in their health funding ``gap''. 
...
hes10_indiv %<>%
  mutate(
    ...
    new_tax = new_income_tax_outer0910(taxable_income, rates = new_0910_tax_rates),
...
hes_with_gst_and_tax_compensation %T>%
...
\begin{tabular}{lrr}
\textbf{Tax bracket} & \textbf{Current tax rate} & \parbox[c]{0.4\columnwidth}{\raggedright\bfseries Budgetary impact of 1\\ percentage point change}\\
\hline
\$0-\$18,200 & 0\% & \\
\$18,201 - \$37,000 & 19\% & \$1.6\\
\$37,001 - \$80,000 & 32.5\% & \$1.9\\
\$80,001 - \$180,000& 37\% & \$1.0\\
\$180,001 + & 45\% & \$0.5\\
\lasthline
\end{tabular}
...
\source{Grattan analysis ATO 2\%\ sample file (2013)}
\end{table}
...
<<BottomQuintileFallingThroughCracks, eval=FALSE>>=
library(party)
library(glmnet)
...
bottom.quintile.not.compensated %>% 
  group_by(age_group) %>%
  summarise(not_compensated = weighted.mean(not_compensated, Weight_HH_HES)) %>%
  arrange(age_group) %>%
  grplot(aes(x = age_group, y = not_compensated)) + 
  geom_bar(stat = "identity")

bottom.quintile.not.compensated %>% 
  group_by(Number_of_dependants_aged_under_years_in_household) %>%
  summarise(not_compensated = weighted.mean(not_compensated, Weight_HH_HES)) %>%
  arrange(Number_of_dependants_aged_under_years_in_household) %>%
  mutate(Number_of_dependants_aged_under_years_in_household = gsub("dependent.*$", "", Number_of_dependants_aged_under_years_in_household)) %>%
  grplot(aes(x = Number_of_dependants_aged_under_years_in_household, y = not_compensated)) + 
  geom_bar(stat = "identity")

bottom.quintile.not.compensated %>%
  grplot(aes(x = not_compensated, y = Prev_fin_year_HH_disposable_income, fill = (age_group))) + 
  geom_boxplot() + 
  geom_text(aes(label = gsub("\\s", "\n", age_group), y = median(Prev_fin_year_HH_disposable_income)), position = position_dodge(width = 0.75)) + 
  scale_fill_manual(values = rev(gpalx(length(unique(bottom.quintile.not.compensated$age_group))))) +
  coord_cartesian(ylim = c(-1e3,100e3))
...
party.cracks <- 
  party::ctree(factor(not_compensated) ~ Year_purchased_dwelling_HH_ranged_factor.ordered + Prev_fin_year_HH_disposable_income + Number_of_dependent_children_aged_under_years_in_household + age_group + Number_of_unemployed_persons_in_the_household + Net_wealth_of_household, 
               controls = ctree_control(minbucket = 100e3),
               weights = round(bottom.quintile.not.compensated_factors$Weight_HH_HES), 
...
rpart.cracks.certain.features <- 
  # aged under **15** years
  rpart(factor(not_compensated) ~ Number_of_dependent_children_aged_under_years_in_household + Prev_fin_year_HH_disposable_income + Current_weekly_HH_disposable_income + age_group, weights = Weight_HH_HES, data = bottom.quintile.not.compensated, control = rpart.control(minbucket = 200))
plot(rpart.cracks)
text(rpart.cracks)
