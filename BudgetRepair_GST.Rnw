...
hes_with_gst_compensation <- 
  ...
  merge(., amount_of_quintile_on_welfare) %>% 
  group_by(gross_income_quintile,
...
\begin{tabular}{lrr}
\textbf{Tax bracket} & \textbf{Current tax rate} & \parbox[c]{0.4\columnwidth}{\raggedleft\bfseries Budgetary impact of 1\\[-2pt] percentage point change}\\
\hline
\$0-\$18,200 & 0\% & \\
...
bottom.quintile.not.compensated <- 
  ...
  mutate(not_compensated = new_welfare + tax_diff_weekly < Weekly_household_GST_on_all_goods_and_services_FIS/2,
         ...
         compensation_inadequacy = ifelse(not_compensated, 
                                          ...
                                          0),
         has_welfare = new_welfare > 0,
         compensation_rel_adequate = Weekly_household_GST_on_all_goods_and_services_FIS/2 - new_welfare - tax_diff_weekly) %>%
...
party.cracks.1 <- 
  party::ctree(factor(not_compensated) ~ Year_purchased_dwelling_HH_ranged_factor.ordered + Prev_fin_year_HH_disposable_income + number_dependent_children + has_children + age_group + Number_of_unemployed_persons_in_the_household + Net_wealth_of_household + has_welfare, 
