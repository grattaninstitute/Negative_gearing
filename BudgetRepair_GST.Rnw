...
<<BottomQuintileFallingThroughCracks>>=
...
bottom.quintile.not.compensated %>%
  mutate(
    ...
    Year_purchased_dwelling_HH_ranged_numeric = ifelse(grepl("before 1960"), "1955",
                                                       ifelse(grepl("after 2008", "2009",
...
bottom.quintile.not.compensated %>%
  ...
  party::ctree(factor(not_compensated) ~ ., data = bottom.quintile.not.compensated)
