...
\begin{itemize}
\item a reduction in marginal income tax rates of nearly one percentage point for every bracket\footnote{Together this would cost around \$5.3~bn, \Vref[see:]{tbl:tax_cuts} on the budgetary costs of income tax rate changes. }
\end{itemize}
