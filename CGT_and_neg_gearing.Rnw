...
historical.interest.losses <- 
  ...
  group_by(year) %>%
  summarise(tot.rent_interest_deds = sum(Rent_int_ded_amt))
...
Investors are also able to choose the time of an asset's sale to minimise taxes on capital gains. We see evidence of people waiting to retirement to realise capital gains (see below). 
...
It is understandable that politicians today accept their predecessors' judgement---that rental prices did rise and that quarantining negative gearing was to blame.
But their predecessors were wrong.  
Although rental prices rose rapidly in Perth somewhat in Sydney, %
inflation-adjusted rental prices were stable in Melbourne and actually fell in Adelaide and Brisbane (\Vref{fig:Capital_city_rents}).
...
Adelaide and Brisbane actually experienced declines.}{Average rent prices (real compared with overall CPI), 1982 = 1.
...
If the buyer is an owner-occupier, the rental market has one fewer house to supply, but also one fewer tenant to satisfy. 
Thus, no change in rent.
