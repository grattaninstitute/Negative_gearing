...
\newcommand\redsout{\bgroup\markoverwith{\textcolor{red}{\rule[0.4ex]{2pt}{3pt}}}\ULon}
...
<<minimal_preamble, echo=FALSE, dev='pdf', echo=FALSE, message=FALSE, warning=FALSE>>=
...
<<preamble, cache=F, echo=FALSE, dev='pdf', echo=FALSE, message=FALSE, warning=FALSE>>=
library(data.table)
library(dplyr)
library(tidyr)
library(magrittr)
...
library(devtools)
library(grattan)
# temp issues with library(XLConnect)
...
\begin{summary}
This working paper examines the odd treatment of capital gains and investment losses by current tax policies. The polices are unnecessary, harmful, and costly. This is a test sentence.

But is this going in?
...
read_excel("ASX_Returns_by_asset_class.xlsx") %>%
  ...
  theme(panel.grid.major = element_blank(),
        plot.margin = grid::unit(c(0.7,2.2,0.1,0.1), "lines"),
