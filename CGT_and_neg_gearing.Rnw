...
<<knitrOpts, echo=FALSE, dev='pdf', echo=FALSE, message=FALSE, warning=FALSE>>=
...
<<CGT_inflator>>=
CG_inflator <- function(from_fy, to_fy){
    ...
    daley_taxstats <- 
      ...
      mutate(
        Taxable_Income = Taxable_Income - Sw_amt + Sw_amt * (wage_inflator(from_fy = "2012-13", to_fy = "2013-14"))^(year - 2013) - 
          Net_CG_amt + Net_CG_amt * CG_inflator(from_fy = "2012-13", to_fy = yr2fy(year)),
        ...
        Net_CG_amt = Net_CG_amt * CG_inflator(from_fy = "2012-13", to_fy = yr2fy(year)),
        Tot_CY_CG_amt = Tot_CY_CG_amt * CG_inflator(from_fy = "2012-13", to_fy = yr2fy(year))
      )
  } else {
    daley_taxstats <- 
...
Reducing the capital gains discount to \Sexpr{100 * new.discount}~per cent and quarantining losses so they can only be written off against relevant investment income could raise around \$\Sexpr{costing_policy_2015_year0}~billion a year in the short-term.
...
n.NGrs <- sample_file_1213 %$% sum(Net_rent_amt < 0) * 50 / 10^6
...
@
\begin{figure}[h]
\Caption{The minority of taxable gains are earned by individuals}{Taxable net capital gains 2012-13 (billions)}{fig:Majority_of_taxable_gains_are_earned_by_individuals}
...
\section{The 50~per cent capital gains tax discount is too generous}\label{sec:ShouldCapitalGainsBeTaxedConsistently}\label{sec:The50pcCGTdiscounttoogenerous}
...
Taxing capital gains consistently with ordinary income maintains the integrity of the tax base. 
Concessions results in ``revenue leakage as taxpayers find ways to convert income into capital gains.\footcites{Evans2005}{Minas2013} Sealing the income tax base is a key reason capital gains are taxed in most OECD countries\footcite{OECD2006b} and motivated the introduction of capital gains in Australia (see \Vref{box:short_history_CGT})
...
Finally, discounted taxation of capital gains favours wealthy taxpayers who receive most of the capital gains. 
...
\begin{smallbox}{Tax gains on an accruals basis?}{box:accrual}
Taxing capital gains on realization creates several distortionary effects, which we have mentioned above. 
Investors are incentived to hold onto assets that they would otherwise sell because of the looming tax bill. 
Investors also enjoy a lengthy deferral of tax, effectively a tax-free loan from the government to  (generally wealthy) asset-holders. 
Finally, realization-based CGT regimes allow asset holders to wait until their marginal tax rate is low (as during retirement) before selling.

These problems can be eliminated by taxing gains on an accruals basis -- that is, by requiring asset holders to pay tax every year the asset is held on the accrued gain, regardless of whether it is sold or not. 
Although taxing on an accrual basis has been recognized as the ideal capital gains tax regime in theory\footnote{Assuming a comprehensive income taxation philosophy; vide: \textcite{burman2009taxing}} it has been neglected by policy-makers due to perceived difficulties in implementation.

Although implementation is always difficult, the claim that taxing on an accruals basis is too impractical is now being questioned in the tax literature.\footnote{See for example \textcite{Ingles2015CGT}.} 
The two main problems with accruals taxation -- determining the change in an asset's price when it hasn't been exposed to the market, and potential low liquidity in asset holders -- are serious enough to warrant focus, but in my opinion are less serious than the current regime's distortions.

Determining the price of shares is trivial and has been advocated\footnote{Somewhere.}.
Accrued prices of property are more difficult to determine; however, price indices are reliable indicators of growth over areas. 
The details how these price indices translate into a formula to calculate accruals are, in my view, unimportant.
Growth residual to these indices may impose a greater tax burden on some owners than is deserved; however, it is equally likely that owners would receive an unfairly low tax bill at the end of accrual period. 
In any case, an investor who chooses to dispose of his asset would only pay tax on the last gain actually realized, which would wash out any discrepancy between the indices and the investor's true gain.

Of greater difficulty are unlisted shares or business assets, which are difficult to value. 


The current regime of capital gains taxation has several well-documnted flaws.
Taxes are imposed on realization, which creates economic distortions through asset lock-in, provides investors with an implicit interest-free loan towards their tax liabilities, and allows investors to minimize their tax by waiting till retirement (when their marginal tax rate is very low) or by avoiding tax altogether.
\end{smallbox}

\subsection{Some discount may be justified but 50~per cent is too high}
A concession from marginal rates of income tax should only be made if it serves a policy aim. 

Some discount on tax treatment of gains is justified to avoid taxing the inflation component of gains.
But with inflation rates low relative to investment returns, the 50~per cent discount overcompensates most investors. 
This is particularly true when the other tax advantages of gains -- tax deferral and the ability to control the timing of realisation -- are taken into account. 
Other justifications for concessional tax treatment of gains -- maintaining the incentives to save and start new business and moderating the effect of ``asset lock-in'' -- have weak empirical foundations. In any case, lower taxes on capital gains are not the most targeted way to achieve these objectives.
...
A lower tax rate for capital gains is sometimes justified on the basis that the inflation part of gains should not be taxed. 
The effective tax rates on real returns can be high if nominal gains are taxed at marginal income tax rates.\footcite{Treasury2010} 
This is particularly true of investments with low real returns that are held for an extended period (\Vref{box:short_history_CGT}).

But the tax concession is a crude way to adjust for the effects of inflation. 
Investors pay less tax compared to a tax on real gains long as asset values grow at least twice the rate of inflation. 
So the discount overcompensates investors with high returns and undercompensates those with low ones. 

Since the introduction of the capital gains tax discount in 1999, house prices have grown annually by an average of 7.3~per cent while share prices have grown by an average of only 3.6~per cent. 
Inflation over this period was 2.8~per cent.\footcite{ABS2015a}  
So the 50~per cent discount has overcompensated housing investors. 
In contrast, share investors, who have suffered low average capital returns due to the global financial crisis, would have been better off under a regime that taxed real capital gains. 

But full compensation for inflation is not needed to constrain effective tax rates on gains at or below an investor's marginal rate.  
Capital gains enjoy other tax advantage that act to lower the effective tax rates. 

First, capital gains are taxed upon realisation (\ie when the asset is sold) not as they accrue, providing investors with an implicit interest-free loan on tax liabilities.\footnote{See also:~\textcites[p.~2]{Fane2004}[p.~12]{Ingles2009}}
This substantially reduces the effective tax rate paid on gains, with the tax benefit increasing the longer the asset is held (\Vref{box:Tax_deferral_and_effective_tax_rates}).\footcite[p.~65]{Treasury2010}  

Investors are also able to choose the time of an asset's sale to minimise taxes on capital gains. We see evidence of people waiting to retirement to realise capital gains (see below).\label{sec:InvestorsChooseTimeOfRealization} 
This reduces the average tax rate an investor pays on capital gains income. 
For example, the average marginal tax rate on capital gains for people in the workforce is 31~per cent compared to 17~per cent for people over 50 with no wage income.\footcite{ATO2013i} 
...
Australia's current system provides no explicit inflation adjustment for interest on bank deposits -- assets disproportionately held by the least well off\footnote{Bank deposits comprise 20\%\ of the assets of the households in the lowest income decile compared with around 5\%\ for top two income deciles. \textcite[Wave~10]{HILDA2015}}  -- that do not receive these other tax advantages. \nocite{*}

The current 50 per cent discount compensates, and in many cases overcompensates, for the effects of inflation on post-tax returns. 
When the other tax advantages for capital gains income are factored in, capital growth assets receive sizeable tax breaks.  
...
both %T>%
  ...
  scale_y_continuous(expand = c(0,0.00), label = percent) + 
  scale_fill_manual(values = gpal(2, T)) + 
  annotate("blank", x = 1, y = 0.7) + 
...
The capital gains tax discount magnifies the tax advantaged status of capital gains. With the discount, effective tax rates fall to 21~per cent after 5 years and 15 per cent after two decades, generating a substantial wedge between the tax treatment of capital gains and recurrent income such as interest on bank deposits. 
\eject
...
\subsubsection{Taxing capital gains without a discount will not substantially deter savings and investment}
...
It is not clear whether deferral of taxes on gains until realisation, itself a significant tax advantage (\vref{sec:InvestorsChooseTimeOfRealization}), is itself enough to compensate investors for this risk. \textcite[p.~8]{Burman2009};  \textcite[p.~130]{Commission2004}}  But this effect is small.
...
The sale will only be worthwhile if Hayley can find an alternative property that yields more than her current rental income of \$50,000 with the same opportunity for capital gains. With her \$926,000 sale proceeds she would need to find a property with net rental return of more than 5.4 per cent a year. 

Properties with rental yields below this but higher than her current 5 per cent would not be attractive because realising the tax liability on her current property erodes her capital base for investment.
...
Negative gearing is the use of losses made on investments (including mortgage interest payments) to reduce one's taxable income including wages. 
...
But there is no theoretical basis for allowing losses on investment to be deducted against entirely unrelated income such as wages. 
Indeed, allowing full deductibility of investment losses against wages when capital gains are taxed concessionally and only on realisation leads investors to favour debt financed investments that generate more of their returns via capital gains in order to maximise tax breaks on wages. 
This reduces tax collections and has negative impacts in investment markets, particularly housing markets, where the tax treatment has encouraged speculative activity. 
...
\subsection{Interaction with capital gains tax allows investors to defer and reduce their tax}
...
Over 70~per cent of those under 60 with investment properties make rental losses compared to less than 35~per cent over 60. (See \Vref{fig:Age_negative_gearing_investments}.)
...
sample_file_1213 %>%
  ...
  mutate(gearing = factor(gearing, 
                          ...
                          labels = c("Negatively geared", "Positively geared", "(Not in property market)"))) %T>%
  chart_data %>%
  grplot(aes(as.numeric(Age), 
             number, 
             fill = gearing,
             order = gearing,
             group = gearing,
...
\source{Grattan analysis}
\end{figure}
The Henry Tax Review described the asymmetry between gains and losses as ``among the greatest tax-induced biases to the savings choices of households.''\footcite[p.~69]{Treasury2010} This runs counter to the rationale for allowing the deductibility of losses -- to maintain tax neutrality of debt and equity financing.
...
While these distortions exist for all investments, the tax advantages are greatest for property because bank lending rules allow greater leverage than for others assets such as shares that also produce capital gains.\footcite[p.~23]{RBA2015}

<<Negative_Gearing_tax_expenditures>>=
get_sample_file_and_year <- function(yr){
  get_sample_file(yr) %>%
    mutate(year = yr)
}
...
NG_tax_exps <-
  lapply(2004:2013, get_sample_file_and_year) %>%
  rbindlist(fill = TRUE) %>%
  tbl_df %>%
  group_by(year) %>%
  mutate(current_tax = income_tax(Taxable_Income, fy.year = yr2fy(year)),
         subseqt_tax = income_tax(ifelse(Net_rent_amt < 0, 
                                         Taxable_Income - Net_rent_amt,
                                         Taxable_Income), fy.year = yr2fy(year)),
         diff = subseqt_tax - current_tax) %>%
  group_by(year) %>%
  summarise(avg.diff = sum(diff) * weighting(year) / 1e9)

NG_tax_exps.real <-
  NG_tax_exps %>%
  ungroup %>%
  mutate(avg.diff.real = cpi_inflator(avg.diff, from_fy = yr2fy(year)))
...
By 2012-13, the two~million landlords reported collective losses of \$5.5 billion (\Vref{fig:Net_rent_over_time}).\footcites{Eslake2013}{Treasury2015a}
...
taxstats_ts %>%
  ...
  mutate(fy.year = factor(fy.year, ordered = TRUE),
         ...
         unit = ifelse(grepl("\\$", unit), "amount", "number")) %T>%
   chart_data %>%
   grplot(aes(x = date, y = value.rel, color = group)) + 
   ...
   scale_x_date(breaks = as.Date(paste0(seq(1980, 2010, by = 10), "-01-01")),
...
taxstats_ts %>%
  ...
  geom_bar(aes(x = year, y = value.real), stat = "identity", width = 1)+
...
\end{figure}
\begin{figure}[p]
\Caption{Growth in the number of negatively geared investors has outpaced the growth in the number of investors. And the growth in the amount of their losses is even greater.}{1999-00 = 1.}{fig:Negative_gearing_real_over_time}
...
\begin{bigbox*}{Impact of negative gearing on house prices}{box:Impact_of_NG_on_house_prices}
Limiting or abolishing negative gearing will reduce the post-tax investment returns for negatively geared property investors. 
There will be a one-off decrease in house prices as investors reduce their willingness to pay. 
Of course, house prices may not actually fall if market dynamics continue to push prices upwards. 
But prices will be lower than they would have been in the absence of the tax changes.

<<Impact_of_NG_on_house_prices, cache = FALSE>>=
get_sample_file_and_year <- function(yr){
  get_sample_file(yr) %>%
    mutate(year = yr)
}
...
rental_losses <- 
  ...
  group_by(year) %>%
  summarise(tot_avg_loss = -1 * sum(Net_rent_amt) * weighting(year)) %>%
  ungroup %>%
  mutate(real_tot_avg_loss = cpi_inflator(tot_avg_loss, from_fy = yr2fy(year)))
...
NG_tax_exps <-
  ...
  mutate(current_tax = income_tax(Taxable_Income, fy.year = yr2fy(year)),
         ...
         diff = subseqt_tax - current_tax) %>%
  group_by(year) %>%
  summarise(avg.diff = sum(diff) * weighting(year))
...
value.of.ng.by.yr <- 
  ...
  mutate(benefit_due_NG = income_tax(pmax(Taxable_Income, Taxable_Income - Net_rent_amt), 
                                     fy.year = yr2fy(year)) -  income_tax(Taxable_Income, 
                                                                          fy.year = yr2fy(year))) %>%
  group_by(year) %>%
  summarise(total_benefit = sum(benefit_due_NG) * weighting(year)) %>%
  ungroup %>%
  mutate(total_benefit_real = cpi_inflator(total_benefit, from_fy = yr2fy(year), to_fy = "2014-15"))
...
The impact on prices depends on investor perceptions of the size of the lost tax benefit. 
Average annual rental losses over the last decade were \$\Sexpr{texNum(mean(rental_losses$real_tot_avg_loss))}, generating annual tax write-offs worth an average of \$\Sexpr{texNum(mean(value.of.ng.by.yr$total_benefit_real))} in today's dollars. 
If investor expectations are guided by history, the present value of tax benefits over the next decade from negative gearing is around \$\Sexpr{texNum(present.value_benefit_from_NG)}. The present value of tax benefits in perpertuity is \$\Sexpr{texNum(mean(value.of.ng.by.yr$total_benefit_real)/RoR_proprty)}.

If negative gearing were entirely abolished, then these expected tax benefits would be lost entirely. 
Given that the value of dwellings held by investors is \$\Sexpr{texNum(total.value.of.property.2014)} (2014), a loss of \$\Sexpr{texNum(present.value_benefit_from_NG)} from the value would correspond to a fall in prices of \Sexpr{pc.prop_val_due_NG}. 
For policies that limit negative gearing the price impacts would be smaller since some of the tax benefits are retained. 
For example, if investment losses are quarantined so they can't be written off against wage and salary income, the present value of tax benefits lost over a decade might be \$\Sexpr{round(present.value_benefit_from_Quaran_NG/1e9)}~billion, with a maximum impact on property prices of \Sexpr{pc.prop_val_due_NG_Quaran}.

Alternatively, assuming that investors demand the same return on investment without negative gearing, the change could be as high as \Sexpr{latex_percent(change_in_price_due_to_NG_upper)}. This is equal to the proportion of the current value of investor residential property in the value of negative gearing as a perpetuity. (The post-tax return on residential property investment was \Sexpr{latex_percent(RoR_proprty)} in the decade to December 2013.)

Let $V_0$ be the current value of (investor, residential) housing stock. Let $V_{10}$ be the value of $V_0$ in ten years' time. Let $V_{0\not{G}}'$ be the new value of housing stock in the absence of negative gearing. Let $G$ be the value of negative gearing in perpertuity. Let $r$ be the rate of return demanded in residential property. So 
\[V_{10} = V_0(1 + r)^{10}\]
and by assumption
\begin{align*}
V_{10} - G &= V_{0\not{G}}' (1 + r)^{10} \\
V_{0\not{G}}' &= \frac{V_{10} - G}{(1 + r)^{10}} \\
              &= \frac{V_0(1 +r)^{10} - G}{(1 + r)^{10}} \\
\implies \frac{V_{0\not G}'}{V_0} &= 1 - \frac{G}{V_0(1 + r)^{10}}
\end{align*}

Thus \[\frac{V_{0\not G}'}{V_0} = \text{\Sexpr{latex_percent(1 - value_of_NG_perpertuity * NG_inflator(1)/(total.value.of.property.2014 * (1 + RoR_proprty)^(1)))}}.\]
\end{bigbox*}

One concern with limiting negative gearing is that downward pressure on house prices could leave some highly geared investors with negative equity.  
But given expected one-off price impacts of less than 
\Sexpr{pc.prop_val_due_NG} 
(\Vref{box:Impact_of_NG_on_house_prices}), the risk of negative equity from the tax changes is limited. 
Only around ten per cent of new property loans go to investors with loan value ratios in excess of 90 per cent.\footcite{APRA2015}  
And ratios for existing loans are falling as property prices continue to grow. 
For the small proportion of investors that remain highly leveraged, higher interest rates pose a far greater risk than changes to the tax treatment of rental losses.

\subsubsection{Limiting negative gearing will not change rents much}
...
<<Capital_city_rents_direct_abs_get, fig.height = 11, out.height = "11in", cache=TRUE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% CHAPTER 3: IMPACT OF NEGATIVE GEARING AND THE CGT %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<Impact, child='./CGT-NG-sections/chapter-Impact-of-changing-negative-gearing/Impact-of-changing-negative-gearing.Rnw'>>=
@
...
Reducing the discount to 30~per cent, could raise around \$\Sexpr{cost_of_red_CGT_discount}~billion.
This estimate relies on a pair of assumptions with opposing effects. The first is that asset prices and investor behaviour would not change and the second is that the 2012-13 taxation statistics, being the most recent, are representative of future years.  The first is likely to result in a higher estimate than could actually be anticipated as a less favourable tax treatment would discourage investors and lower prices. On the other hand, the second assumption would underestimate the revenue gain these revenues should rise over time  as capital losses built up from the GFC pass were yet to pass through the system.\footnote{CGT revenues are yet to recover following the GFC.
Receipts were 0.46~per cent of GDP in 2012-13, down from a peak of 1.56~per cent of GDP in 2007-08.
Even as asset prices have improved, capital losses carried forward have limited taxable gains. \textcite{Stewart2015}; \textcite{PBO2014}. } In our judgement, the opposing effects are likely to be of similar magnitude.\footnote{An independent costing by the Parliamentary Budget Office on behalf of Sen. Di Natale this year yielded similar forecasts: a 25 percentage point reduction in the CGT discount resulted in a forecast increase in revenue of \$5.7~bn over three years, looking at a different sample year and accounting for reductions in asset investment due to discouraged investor behaviour (but not changes in asset prices). Our analysis suggests a \$1.9~bn (or \$5.7~bn over three years) under the same CGT discount change.}
...
sample_file_1213 %>%
  ...
  tally %>%
  ungroup %>% 
  mutate(alpha = (n - min(n))/(max(n) - min(n))) %>%
...
policy %>%
  ...
  geom_tile(fill = Orange) +
  coord_equal() + 
  guides(alpha = guide_legend("Increased tax burden")) + 
  theme(legend.position = "right",
        panel.grid = element_blank(),
        axis.title.x = element_text(hjust = 0.5, angle = 0))
@
\begin{figure}
\Caption{The costs of this policy fall on the high income and the elderly}{Total income decile}{fig:income_age_heatmap}
\includegraphics[height=0.9\textheight]{figure/income_age_heatmap_costing-1}
\end{figure}

<<income_age_heatmap_just_NG>>=
just_negative_gearing_taxstats_year0 %>%
  mutate(diff = new_tax - current_tax) %>%
  age_mutate_ %>%
  # mutate(Income_decile = ntile(Tot_inc_amt, 10))
  group_by(Income_decile = ntile(Tot_inc_amt, 10),
           Age) %>% 
  summarise(mean.diff = mean(diff)) %>%
  ungroup %>% 
