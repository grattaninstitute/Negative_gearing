...
\begin{smallbox}{Impact of negative gearing on house prices}{box:Impact_of_NG_on_house_prices}
...
rental_losses <- 
  ...
  group_by(year) %>%
  summarise(real_tot_avg_loss = cpi_inflator(sum(Net_rent_amt), from_fy = yr2fy(year)))
...
present.value_benefit_from_NG <-
  grattan::npv(rate = 0.05, values = rep(NG_tax_exps.real, 100))

present.value_benefit_from_Quaran_NG <-
  -round(grattan::pv(0.05, 10, sum(just_negative_gearing_taxstats$diff)*weighting(2015)))
...
cls <- slot(metadata, "codelists")
codelists <- sapply(slot(cls, "codelists"), function(x) slot(x, "id")) 

recode_res_dwelling_values <- function(DT, colname){
  colname.code <- 
    as.data.frame(slot(metadata, "codelists"), 
...
# MEASURE.code <- 
#   as.data.frame(slot(metadata, "codelists"), 
#                 codelistId = codelists[grep("MEASURE", codelists)]) %>%
#   select(-ends_with("fr")) # damn French
# 
# ASGS_2011.code <- 
#   as.data.frame(slot(metadata, "codelists"), 
#                 codelistId = codelists[grep("ASGS_2011", codelists)]) %>%
#   select(-ends_with("fr"))
...
#   merge(MEASURE.code, by.x = "MEASURE", by.y = "id") %>%
#   merge(ASGS_2011.code, by.x = "ASGS_2011", by.y = "id") %>%
  as.data.table


metadata_code_ds <- 
  as.data.frame(metadata[[which(names(metadata) == "CodeLists")]]) %>%
  gather(var, val, -CodeList.Name.text) %>% 
  head
  setnames(old = names(.))
...
pc.prop_val_due_NG <- 
  paste0(round(100*present.value_benefit_from_NG / total.value.of.property.2014, 1), "\\%")
pc.prop_val_due_NG_Quaran <-
  paste0(round(100*present.value_benefit_from_Quaran_NG / total.value.of.property.2014, 1), "\\%")
@

The impact on prices depends on investor perceptions of the size of the lost tax benefit. 
Average annual rental losses over the last decade were \$12.8 billion, generating annual tax write-offs worth an average of \$4 billion in today's dollars. 
If investor expectations are guided by history, the present value of tax benefits over the next decade from negative gearing is around \Sexpr{round(present.value_benefit_from_NG/10^9)}~billion. 

If negative gearing were entirely abolished, then these expected tax benefits would be lost entirely. 
If this were fully capitalised in the price of property, prices could fall by up to \Sexpr{pc.prop_val_due_NG}. 
