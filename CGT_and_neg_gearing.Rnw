...
taxstats_ts %>%
  ...
  annotate("text",
           ...
           x = as.Date(paste0(c(1985, 2010, 2010), "-06-30")),
           y = c(0.5, 3.3, 1.8),
           lineheight = 0.8,
           hjust = 0,
           vjust = c(1.25,0,0),
...
#   geom_dl(aes(label = group), 
#           method = list("last.qp", dl.trans(cex = 1.875, x = x + 0.0
#                                            #, fontfamily = "Arial"
#                                            )), 
#           font = "bold",
#           hjust = 0) + 
...
For example, if investment losses are quarantined so they can't be written off against wage and salary income, the present value of tax benefits lost over a decade might be \$\Sexpr{round(grattan::pv(0.05, 10, just_negative_gearing_taxstats))}, with a maximum impact on property prices of X\%.
