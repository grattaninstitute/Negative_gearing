...
henry_taxstats <- function(year = 2013, new.discount = 0.30){
  if (year >= 2014){
    CG_inflator <-
      read_excel("Tax-Expenditures-CGT Historical.xlsx") %>%
      select(FY, CGT_discount_for_individuals_and_trusts_millions) %>%
...
value_of_NG_10yrs <-
  -1 * grattan::pv(0.042, 10, value_of_NG_one_year)
...
If investor expectations are guided by history, the present value of tax benefits over the next decade from negative gearing is around \$\Sexpr{texNum(present.value_benefit_from_NG)}. 

If negative gearing were entirely abolished, then these expected tax benefits would be lost entirely. 
Given that the value of dwellings held by investors is \$\Sexpr{texNum(total.value.of.property.2014)} (2014), a loss of \$\Sexpr{texNum(present.value_benefit_from_NG)} could see prices fall by up to \Sexpr{pc.prop_val_due_NG}. 
...
Alternatively, assuming that investors demand the same return on investment without negative gearing, the change could be as high as \Sexpr{latex_percent(change_in_price_due_to_NG_upper)}. The post-tax return on residential property investment was \Sexpr{latex_percent(RoR_proprty)} in the decade to December 2013.
...
Thus \[\frac{V_{0\not G}'}{V_0} = \text{\Sexpr{latex_percent(1 - value_of_NG_perpertuity * NG_inflator(10)/(total.value.of.property.2014 * (1 + RoR_proprty)^(10)))}}.\]
\end{bigbox*}
