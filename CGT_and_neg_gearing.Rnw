...
policy <- cost_of_daley_policy_fast(2015, CGT.discount = 0.40) 
policy %>%
...
total.value.of.property.2014 <- 8223900 * 10^6
...
But given expected one-off price impacts of less than 4 per cent (\Vref{box:Impact_of_NG_on_house_prices}), the risk of negative equity from the tax changes is limited. 
...
housing_finance_commitments <-
  ...
  setkey(series_id) %>%
  merge(housing_finance_commitments.metadata, by = "series_id")
@
...
<<total_value_of_land_australia>>=
present.value_benefit_from_NG / total.value.of.property.2014
@
...
If investor expectations are guided by history, the present value of tax benefits over the next decade from negative gearing is around \$30 billion. 

If negative gearing were entirely abolished, then these expected tax benefits would be lost entirely. 
If this were fully capitalised in the price of property, prices could fall by up to 4 per cent. 
For policies that limit negative gearing the price impacts would be smaller since some of the tax benefits are retained. 
For example, if investment losses are quarantined so they can't be written off against wage and salary income, the present value of tax benefits lost over a decade might be \$\Sexpr{-round(grattan::pv(0.05, 10, sum(just_negative_gearing_taxstats$diff)*weighting(2015)/1e9))}~billion, with a maximum impact on property prices of \Sexpr{"\\%X"}.
