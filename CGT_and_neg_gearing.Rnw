...
costing_policy_2015_year0 <-
  cost_of_daley_policy_fast(2015, eventual = 1) %$%
  round(sum(diff) * 50 / 1e9, 1) 
...
get_sample_file(2013) %$% sum(Net_rent_amt < 0) * 50 / 10^6 -> n.NGrs
...
read_excel("ASX_Returns_by_asset_class.xlsx") %>%
  ...
  mutate(tax_class = paste0(tax_class, "   ")) %>%
...
taxstats_ts %>%
  ...
  geom_dl(method = list("lines2", dl.trans(cex = 1.875, x = x + 0.0))) + 
...
The second arguments reflects a confusion between changes in the size of the investor market and changes in the size of housing market as a whole.
...
<<>>=
rsdmx::readSDMX("http://stat.abs.gov.au/restsdmx/sdmx.ashx/GetData/CPI/2.1+2+3+4+5+6+7+8+50.115522.10.Q/ABS?startTime=1948&endTime=2015") %>% 
  as.data.frame -> cpi.df

rsdmx::readSDMX("http://stat.abs.gov.au/restsdmx/sdmx.ashx/GetData/CPI/1.1+2+3+4+5+6+7+8+50.30014.10.Q/ABS?startTime=1982&endTime=1990") %>% 
  as.data.frame ->
  rent.indices
...
rent.indices %>%
  ...
  grplot(aes(x = as.POSIXct(as.yearqtr(obsTime, format = "%Y-Q%q")),
             y = obsValuerel, 
             color = city,
             label = city,
             group = city)) + 
...
cpi.df %>%
  #data.table %>%
  filter(obsTime >= "1983-Q1", obsTime <= "1990") %>%
  group_by(REGION) %>% 
  # precentage change from prev. period
  mutate(growth_r = (1 + obsValue/100),
         level = lag(cumprod(growth_r)),
         level = ifelse(is.na(level), 1, level)) %>%
  mutate(obsValueReal = cpi_inflator_quarters(level, nominal.date = obsTime, target.date = first(obsTime)),
         obsTime = as.yearqtr(obsTime, format = "%Y-Q%q")) %>%
  ungroup %>% 
  merge(region.decode, by = "REGION") %>% 
  filter(city %in% c("Brisbane", "Adelaide", "Melbourne", "Perth", "Sydney")) %>%
  grplot(aes(x = as.POSIXct(obsTime),
             y = obsValueReal,
             color = city,
             group = city,
             label = city)) + 
