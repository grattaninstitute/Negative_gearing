...
cost_of_daley_policy <- function(year = 2013, eventual = 20, CGT.discount = 0.30){
  ...
  prop_full_discount <-
    ...
    filter(Net_CG_amt > 0) %>%
    group_by(apparent.discount = round(4*Net_CG_amt/Tot_CY_CG_amt,1)) %>%
    tally() %>%
  ...
  mutate(randCG = runif(.)) %>%
    ...
    mutate(discount.key = max(prop_full_discount$discount.key[prop_full_discount$discount.key < randCG])) %>%
    ungroup %>%
