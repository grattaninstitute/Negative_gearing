...
# It's important to note
  ...
  mutate(randCG = runif(.)) %>%
    # If the dice rolls between the discount.keys in prop_full_discount 
    # we choose the lowest prop
    mutate(discount.key = pmin(prop_full_discount$discount.key[prop_full_discount$discount.key > randCG])) %>%
    merge(prop_full_discount, by = "discount.key") %>%
    # avoid NaNs when we 1/apparent.discount
    mutate(apparent.discount = ifelse(apparent.discount < 0.1, 0.05, apparent.discount)) %>%
    mutate(
...
get_sample_file(2013) %$% sum(Net_rent_amt < 0) * 50 / 10^6 -> no.NGrs
no.NGrs <- round(no.NGrs, 1)
@

Negative gearing is embedded in the Australian taxation system; \Sexpr{no.NGrs}~million taxpayers benefit from this policy.
...
The second arguments reflects a confusion between changes in the investor market and changes in the housing market as a whole.
