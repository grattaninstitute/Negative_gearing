...
rent.indices %>%
  ...
  mutate(city = factor(city, 
                       ...
                       labels = c("Sydney", "Melbourne", "Brisbane", "Adelaide", "Perth", "Australian\ncapitals"))) %>%
  mutate(obsTime = as.POSIXct(as.yearqtr(obsTime, format = "%Y-Q%q"))) ->
  rent.indices.real

rent.indices.real  %>%
  filter(obsTime < as.POSIXct("1989-01-01")) %>%
  ...
  annotate("rect", 
           ...
           alpha = 0.5) + 
  geom_line() + 
  geom_text(data = filter(rent.indices.real, obsTime == unique(obsTime)[13]),
            hjust = 1, vjust = -1.1, size = 7.14, fontface = "bold",
            lineheight = 0.8) +
