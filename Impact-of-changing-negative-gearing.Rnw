...
<<Capital_city_rents_direct_abs_get, eval=FALSE>>=
...
saveRDS(rent.indices.real, "rent_indices_real.rds")
@
...
cost_NG_limited_to <- function(Net_rent_amt.limit){
  ...
  target_sample_file %>%
    mutate(new_Net_rent_amt = pmaxC(Net_rent_amt, Net_rent_amt.limit),
           ...
           prev_tax = grattan:::.income_tax(Taxable_Income, "2015-16"),
           post_tax = grattan:::.income_tax(new_Taxable_Income, "2015-16"),
...
\begin{table}
\caption{Budgetary impact of caps to negative gearing}
...
\begin{table}[!h]
\begin{center}
<<HILDA-Number-of-properties, results='asis'>>=
hilda_w14_hh %>%
...
<<Import-SIH>>=
sih201314 <- 
  read.dta("SIH201314/sih13bh.dta")
@
