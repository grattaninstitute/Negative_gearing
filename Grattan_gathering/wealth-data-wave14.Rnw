...
hilda.w14.hh %>%
  select(Household_wealth_Total_superannuation,
         ...
         temp.id) %>%
  mutate(Household_wealth = Household_Net_Worth.p - Household_Net_Worth.n,
         Other_wealth.ii  = Household_wealth - Household_wealth_Total_superannuation - Household_wealth_Equity_investments.i - Household_wealth_Total_property_Apportioned_value.i,
         Household_wealth_decile = .bincode(Household_wealth, 
                                            breaks = household.wealth.deciles,
                                            include.lowest = TRUE)) %>%
  filter(!is.na(Household_wealth_decile)) %>%
  ...
  theme(legend.position = c(0.2125,0.875),
...
read_hilda_get_inheritance <- function(filename){
  ...
  dat <- fread(filename, select = paste0(wave, c("oifinha",  # inheritance
                                                  "hhiage",  # Age last birthday
                                                  "hhwtrp"  # population weight
                                                 )))
  setnames(dat, old = names(dat), new = c("Inheritance", "Age", "Pop_WT"))
  for (j in 1:3)
...
all_years_inheritance %<>%
  ...
  mutate(Inheritance = cpi_inflator(Inheritance, from_fy = yr2fy(year), to_fy = "2014-15")) 
