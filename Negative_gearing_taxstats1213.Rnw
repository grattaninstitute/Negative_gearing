...
cost_of_daley_policy <- function(year = 2013, eventual = 20){
...
individuals_CG <- 
  taxstats_ts %>% 
  mutate(Selected_items = `Selected items`, `Selected items` = NULL) %>% 
  filter(grepl(pattern = "Net capital", Selected_items)) %>% 
  filter(grepl("\\$", unit)) %>% mutate(year = as.numeric(gsub("^.*([0-9]{4}).*$", "\\1", fy.year)) + 1) %>%
