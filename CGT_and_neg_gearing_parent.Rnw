...
data.table::fwrite(hilda_all_waves_rp, file.path = "../Data/HILDA/Wave14/hilda-all-waves-rp.csv")
@
...
Australian entities -- individuals, companies, and superannuation funds -- accrued \Sexpr{texNum(total_CGT, dollar = TRUE)} of income through capital gains in 2013-14. Individuals accrued \Sexpr{latex_percent(cgt_by_entity %>% filter(Entity == "Individuals") %$% prop)}
...
data.table::CJ(
  ...
  arrange(`Nominal annual capital gain__5.0\\%`) %>%
  ungroup %>%
