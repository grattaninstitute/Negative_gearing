...
# TARGET.FY <- "2015-16"
# use.sample.fy <- "2013-14"
revenue_quarantine_NG <- function(sample_file, new_CGT_discount = new.discount, sample_file_fy_year = "2013-14", target_fy = TARGET.FY){
  sample_file %>%
    project_to(to_fy = TARGET.FY, fy.year.of.sample.file = "2012-13") %>%
    # We assume that the CGT discount is enjoyed by everyone with a capital gains
    mutate(new_Net_CG_amt = Net_CG_amt)
}
@
