...
knitr::opts_chunk$set(fig.width=11.000, 
                      ...
                      ,error=FALSE
                      ,results='hide'
                      ,cache=cache_most # override by chunk options
...
<<Import-HILDA-rp, eval=reload_all_data, cache=FALSE>>=
hilda_w14_rp <- 
...
read_hilda_file <- 
  function(filename, max_wave = 14){
    temp <- dplyr::mutate(data.table::data.table(foreign::read.dta(filename)),
    ...
    setnames(temp, old = nms, new = gsub(paste0("^[a-", letters[max_wave], "]"), "", nms))
...
<<Import-HILDA-all-waves-step-2, eval=reload_all_data, cache=FALSE>>=
hilda_all_waves_rp <- 
  rbindlist(hilda_list, use.names = TRUE, fill = TRUE)

write_csv(hilda_all_waves_rp, "../Data/HILDA/Wave14/hilda-all-waves-rp.csv")
@

<<Use-prepared-hilda-all-waves, eval=!reload_all_data>>=
hilda_all_waves_rp <- 
  fread("../Data/HILDA/Wave14/hilda-all-waves-rp.csv", 
        select = c("xwaveid", "wave", "Year", "oifrntp", "lnwtrp"))
@
...
if (!file.exists("impact_policy_after_tax_returns_weighted_all.rda")){
  impact_policy_after_tax_returns_weighted_all <- "!!! unknown ! re-run knitr !!!"
} else {
...
sample_file_1314 %>%
  ...
  group_by(Occupation_description) %>%
  mutate(salary_difference = ifelse(Claiming_PP == "Not claiming", 
                                    average_salary, 
...
@
<<PP-losers-salary-comparison-horiz-bar, cache=FALSE>>=
sample_file_1314 %>%
  merge(occupation_decoder, by = "Occ_code") %>%
  mutate(Sw_amt = as.double(Sw_amt)) %>%  # integer overflow on mean
  group_by(Occupation_description, Claiming_PP =  factor(ifelse(Net_PP_BI_amt < 0, 
...
sample_file_1314 %>%
  ...
  mutate(Claiming = factor(ifelse(grepl("_n_claiming", Claiming), 
...
sample_file_1314 %>%
  ...
  mutate(Sw_amt = as.double(Sw_amt), 
         Claiming_PP =  factor(ifelse(Net_PP_BI_amt < 0, "Claiming PP losses", "Not claiming"), 
                               levels = c("Claiming PP losses", "Not claiming"))) %>%
  # cosmetic
...
xwaveid_with_NG_then_profit <- 
  ...
  select(xwaveid, 
         ...
         oifrntp, 
         lnwtrp) %>%
  rename(FY_rental_property_profit_or_loss = oifrntp) %>%
  mutate(Profit_loss = ifelse(grepl("Loss", FY_rental_property_profit_or_loss, fixed = TRUE), 
                              "Loss", 
                              ifelse(grepl("Profit", FY_rental_property_profit_or_loss, fixed = TRUE),
                                     "Profit", 
                                     ifelse(grepl("Break.even", FY_rental_property_profit_or_loss), 
                                            "Break even", 
                                            NA_character_)))) %>%
  setkey(xwaveid) %>%
  group_by(xwaveid) %>%
  mutate(filled_down = na.locf(Profit_loss, na.rm = FALSE)) %>%
  group_by(xwaveid) %>%
  # This will omit right censoring!!
  filter(any(grepl("Loss", filled_down) & TRUE | (lead(filled_down, order_by = Year, default = "Loss") %in% c("Profit", "Break even"))), na.rm = TRUE)
  ...
  mutate(Profit_loss = ifelse(grepl("Loss", FY_rental_property_profit_or_loss, fixed = TRUE), 
                              ...
                              ifelse(grepl("Profit", FY_rental_property_profit_or_loss, fixed = TRUE),
                                     ...
                                     ifelse(grepl("Break.even", FY_rental_property_profit_or_loss), 
                                            ...
                                            NA_character_)))) %>%
  group_by(xwaveid) %>%
  filter(any(grepl("Loss", FY_rental_property_profit_or_loss, fixed = TRUE))) %>% invisible

hilda_rp_who_negatively_gearing <-
  xwaveid_with_NG_then_profit %>%
  mutate(
    # Returns NA by design
    loss_event = Profit_loss %in% c("Profit", "Break even") & lag(Profit_loss) == "Loss",
    fill_down = na.locf(Profit_loss, na.rm = FALSE), 
    fill_up   = na.locf(Profit_loss, na.rm = FALSE, fromLast = TRUE), 
    left_censored = ifelse(isTRUE(Year == min(Year)), 
                           TRUE, 
                           ifelse(!is.na(first(fill_up, order_by = Year)), 
                                  first(fill_up, order_by = Year) == "Loss", 
                                  FALSE)
                                  ), 
    right_censored = ifelse(isTRUE(Year == max(Year)), 
                            TRUE, 
                            ifelse(!is.na(last(fill_down, order_by = Year)), 
    ...
    # event_interval_optimistic = ifelse(loss_event,
    #                                    1, 
    #                                    
    # )
...
weights_by_waveid <- 
  hilda_rp_who_negatively_gearing %>% 
  select(xwaveid, Year, lnwtrp) %>% 
  group_by(xwaveid) %>% 
  summarise(lon_weight = last(lnwtrp, order_by = Year)) %>%
  # Also, is this right?
  filter(!is.na(lon_weight)) %>%
  setkey(xwaveid)
...
hilda_rp_who_negatively_gearing %>%
  group_by(xwaveid) %>%
  mutate(
    is_interval = is.na(Profit_loss) & !is.na(fill_up) & !is.na(fill_down),
    is_clear    = (is_interval & fill_up == fill_down) | !is_interval,
    optimistic  = ifelse(!is_interval, 
                         Profit_loss,
                         ifelse(is_clear,
                                # i.e. Loss NA Loss ==> Loss Loss Loss
                                fill_up, 
                                "Loss")), 
    pessimistic = ifelse(!is_interval, 
                         Profit_loss,
                         ifelse(is_clear,
                                # i.e. Loss NA Loss ==> Loss Loss Loss
                                fill_up, 
                                "Profit")), 
    gaps_filled = sum(!is_clear),
    CONDO = optimistic == "Loss" & !is.na(optimistic),
    CONDP = pessimistic == "Loss" & !is.na(pessimistic),
    
    years_with_losses_optimistic = cumsum_reset(optimistic == "Loss" & !is.na(optimistic)), 
    years_with_losses_pessimistic = cumsum_reset(pessimistic == "Loss" & !is.na(pessimistic)), 
    years_total = cumsum(!is.na(optimistic))
  ...
  summarise(
    ...
    censor_status = ifelse(any(right_censored), 1, 2)
    )  %T>% 
  {
    hilda_surv_data <<- .
  } %>%
  group_by(xwaveid) %>%
  summarise(hazard_rate_optimistic = optimistic_avg_spell / years_at_risk, 
