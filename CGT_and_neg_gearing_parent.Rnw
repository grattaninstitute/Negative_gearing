...
<<Import-HILDA-rp, cache=FALSE>>=
hilda_w14_rp <- 
  foreign::read.dta("../Data/HILDA/Wave14/Rperson_n140c.dta")
@

<<Import-HILDA-all-waves-step-1, cache=FALSE>>=
read_hilda_file <- 
...
<<Import-HILDA-all-waves-step-2, cache=FALSE>>=
hilda_all_waves_rp <- 
...
sample_file_1314 %>%
  ...
  mutate(`Average salary` = average_salary) %>%
  {
    grplot(., aes(x = Occupation_description, y = `Average salary`, fill = Claiming_PP)) + 
...
\begin{figure}
\Caption{Relative salaries are higher }
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/density-salary-by-PP-losses-1}
\end{figure}
...
@
\begin{figure}
\Caption{Negative gearing spells are short}{Survival curve of negative gearing responding persons}
