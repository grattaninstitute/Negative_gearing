...
# TARGET.FY <- "2015-16"
# use.sample.fy <- "2013-14"
revenue_quarantine_NG <- function(h = 10L, sample_file = sample_file_1314, new_CGT_discount = new.discount, sample_file_fy_year = use.sample.fy, target_fy = TARGET.FY, Z = 50){
  ...
  cum.ecdf <- cumsum(c(1, rep(0, Z)) %*% P)

  
  accumulator <- 
    data.table(rand = cum.ecdf, 
               loss_multiplier = c(seq(0L, Z - 1, by = 1L), 0)) %>%
    setkey(rand)
  ...
  sample_file_1314 %>%
    ...
    mutate(
      ...
      # not salary, CG, or rental
      Other_Income = round(Tot_inc_amt - Sw_amt - Net_CG_amt - Net_rent_amt, 2),
      new_Tot_inc_amt_no_CG = Sw_amt + ifelse(Other_Income <= 0, 
    ...
    mutate(
      accumulated_losses_year_0 = round(-1 * (new_Tot_inc_amt_no_CG - (Tot_inc_amt - Net_CG_amt)), 4), 
  ...
  %>% select(Sw_amt, contains("net"), contains("new"), Taxable_Income, Other_Income, accumulated_losses_year_0, Tot_inc_amt) %>% .[]
