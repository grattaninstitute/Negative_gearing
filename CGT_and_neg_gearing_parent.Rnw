...
revenue_CGT_discount <- function(CGT.discount = new.discount, .old.discount = 0.50){
    ...
    mutate(new_Net_CG_amt = (Net_CG_amt / (1 - .old.discount)) * (1 - CGT.discount),
           ...
           old_tax = grattan:::.income_tax(Taxable_Income, fy.year = fy.year),
           new_tax = grattan:::.income_tax(new_Taxable_Income, fy.year = fy.year),
  ...
  target_sample_file %>%
    ...
    mutate(
      ...
      new_Taxable_Income_year0 = pmaxC(pmaxC(Other_Income + new_Net_CG_amt, 
                                       ...
                                       0),
      prev_tax = grattan:::.income_tax(Taxable_Income, fy.year = "2013-14"), 
      post_tax_yearZ = grattan:::.income_tax(new_Taxable_Income_yearZ, fy.year = "2013-14"), 
      post_tax_year0 = grattan:::.income_tax(new_Taxable_Income_year0, fy.year = "2013-14"),
...
henry_taxstats <- function(new.discount){
  target_sample_file %>%
    mutate(new_capital_gains = 2 * Net_CG_amt * (1 - new.discount), 
           ...
           prev_tax = grattan:::.income_tax(Taxable_Income, fy.year = TARGET.FY), 
           post_tax = grattan:::.income_tax(new_Taxable_Income, fy.year = TARGET.FY), 
           tax_increase = post_tax - prev_tax)
}
@
