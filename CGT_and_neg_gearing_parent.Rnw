...
revenue_CGT_discount <- function(CGT.discount = new.discount, .old.discount = 0.50){
    ...
    mutate(
      new_Net_CG_amt = (1 - cgt_reducer_due_to_policy) * ifelse(runif(n()) < probNoDiscount,
                                                                Net_CG_amt,
...
revenue_quarantine_NG <- function(sample_file = sample_file_1314, new_CGT_discount = new.discount, sample_file_fy_year = use.sample.fy, target_fy = TARGET.FY, age_based = TRUE, Z = 20, probNG = NULL){
  if (sample_file_fy_year != "2013-14"){
  ...
  target_sample_file %>%
    ...
    mutate(
      ...
      new_Net_CG_amt = (1 - cgt_reducer_due_to_policy) * Net_CG_amt * ifelse(runif(n()) < probNoDiscount, 
                                                                             1, 
...
revenue_quarantine_NG_2 <- function(sample_file = sample_file_1314, new_CGT_discount = new.discount, sample_file_fy_year = use.sample.fy, target_fy = TARGET.FY, age_based = TRUE, Z = 20){
  if (sample_file_fy_year != "2013-14"){
    stop("You must correct the sample_file year")
  }
  ...
  probCG_by_age <- 
    target_sample_file %>%
    filter(Gross_rent_amt > 0) %>%
    group_by(age = floor(age_imp)) %>%
    summarise(probCG_overall = mean(Net_CG_amt > 0), 
              # after discount
  ...
  cg_by_age <- 
    target_sample_file %>% 
    filter(Net_CG_amt > 0 | Gross_rent_amt > 0) %>%
    group_by(age = floor(age_imp)) %>%
    summarise(probCG_overall = mean(Net_CG_amt > 0), 
              total_cg = (1 - cgt_reducer_due_to_policy) *sum(Tot_CY_CG_amt),
              expected_cg = (1 - cgt_reducer_due_to_policy) * mean(Tot_CY_CG_amt), 
              expected_tot_cg_given_cg_occurs = (1 - cgt_reducer_due_to_policy) * mean(Tot_CY_CG_amt[Net_CG_amt > 0])) %>%

    ungroup %>%
    arrange(age) 
  
  
  marginal_tax_rate_CG <- 
    sample_file_1314 %>%
    filter(Net_CG_amt > 0) %>%
    mutate(marginal_rate = income_tax(Taxable_Income + 1, "2013-14") - income_tax(Taxable_Income, "2013-14")) %$%
    mean(marginal_rate)
  ...
  if (!age_based){
    ...
    q_by_age <- 
      ...
      filter(Gross_rent_amt > 0) %>%
      group_by(age_range) %>%
      summarise(q = mean(Net_rent_amt < 0) * (1 - mean(Net_CG_amt > 0)))
    ...
    probCG__by_age <- 
      target_sample_file %>%
      group_by(age_at_Z = floor(age_imp)) %>%
      summarise(prob__CG_event = mean(Tot_CY_CG_amt > 0)) %>%
      ungroup %>%
      setkey(age_at_Z)
  ...
  target_sample_file %>%
    select(Net_CG_amt, Sw_amt, ETP_txbl_amt, 
    ...
    mutate(
      ...
      accumulated_losses_year_0 = -1 * ifelse(Net_rent_amt >= 0, 
                                              0, 
                                              pminC(pmaxC(Other_Income, 0) + Net_rent_amt, 0)) 
      # Not required.
      # ,new_Tot_inc_amt = new_Tot_inc_amt_no_CG + Net_CG_amt
    ) %>% 
    mutate(rand = runif(n()), 
    ...
    mutate(
      income_reduction = ifelse(rand < prob__CG_event & accumulated_losses_year_Z > 0, 
      ...
      income_reduction = pminV(40e3, accumulated_losses_year_Z),
      gain_residual = 0* ifelse(rand < prob__CG_event & accumulated_losses_year_Z > 0,
    ...
    mutate(
      ...
      prev_tax = income_tax(pmaxC(Taxable_Income, 0), fy.year = "2015-16"), 
      post_tax_yearZ = income_tax(pmaxC(new_Taxable_Income_yearZ, 0),
                                        fy.year = "2015-16"), 
      post_tax_year0 = income_tax(new_Taxable_Income_year0, fy.year = "2015-16"),
      minimum_taxable_income = pmaxC(quarantined_income + new_Net_CG_amt - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed, 
                                     0),
      tax_increase = post_tax_yearZ - prev_tax, 
      tax_increase_year0 = post_tax_year0 - prev_tax, 
      residual_gain_effect = income_tax(pmaxC(pmaxC(Other_Income + new_Net_CG_amt - income_reduction, 
                                                    0) + quarantined_income - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed, 
                                              0),
                                        fy.year = "2015-16") - post_tax_yearZ
    )
}
@
...
henry_taxstats <- function(new.discount){
  target_sample_file %>%
    mutate(new_capital_gains = 2 * Net_CG_amt * (1 - new.discount), 
           ...
           new_Taxable_Income = pmaxC(Taxable_Income - Net_CG_amt + new_capital_gains - Net_rent_amt + new_net_rent - Net_fincl_invstmt_lss_amt + new_investment_lss, 
                                      0), 
...
policy_alternate_method <- 
  revenue_quarantine_NG_2(sample_file = sample_file_1314, 
                          new_CGT_discount = 0.25, 
                          sample_file_fy_year = "2013-14", 
                          target_fy = TARGET.FY, 
                          Z = 50)
...
revenue_foregone_CG_and_NG <- 
  cost_of(revenue_abolish_NG()) + cost_of(revenue_CGT_discount(CGT.discount = 0))
@

<<CostofNGtotal>>=
sample_file_1314 %>%
  mutate(new_tax = income_tax(Taxable_Income - pmin(0, Net_rent_amt), fy.year = "2013-14"),
         diff = new_tax - income_tax(Taxable_Income, fy.year = "2013-14")) %$%
  sum(diff) * 50 / 1e9  ->
  NG.tx.exp
...
emtr_rental <- function(gearing_ratio, return_type = c("real", "nominal", "excess"), n = 15, CGT.discount = 0.50, inflation = 0.025, risk.free.rate = inflation + 0.02, marginal.tax.rate = 0.47, investment.value = 100, nominal.rent = 0.03, capital.gains = 0.05, interest.rate = 0.05, cgt_indexed = FALSE){
  # 
  # # For debugging
  # gearing_ratio = 0.80
  ...
  for (y in 2:n){
    ...
    if (!cgt_indexed){
      ...
      CGT_if_gain_realized <- (nominal_gain_since_inception / inflation_index) * marginal.tax.rate
    }
    
    nominal_investment_value_before_tax <- investment.value * (1 + nominal.rent + capital.gains) ^ y
    nominal_investment_value_after_tax <- 
      carry_forward_balance + rent - tax_on_rent_minus_interest + annual_capital_gains - CGT_if_gain_realized
...
data.table::CJ(
  ...
  mutate(marginal_tax_rate = 0.47, 
         marginal_tax_rate_label = ifelse(return_type == "nominal" & scenario == "historic", 
                                          "Nominal tax rate", 
                                          NA_character_)) %>%
  rowwise %>%
  mutate(
  ...
  geom_hline(aes(yintercept = marginal_tax_rate), linetype = "dashed") + 
  geom_text(aes(label = marginal_tax_rate_label, y = marginal_tax_rate), 
            nudge_y = 0.05, # position = position_dodge(width = 0.9), 
            nudge_x = -0.5,
            size = 22/(14/5),
            color = DarkOrange,
            vjust = -0.1, 
...
emtr_nominal_historic_lower_assumptions <- function(){
  ...
  data.table::CJ(
    ...
    mutate(marginal_tax_rate = 0.47) %>%
    rowwise %>%
    mutate(return_type = return_type,  
           marginal.tax.rate = marginal_tax_rate, 
           capital.gains = if (scenario == "historic") 0.073 else 0.050,
           risk.free.rate = if (scenario == "historic") 0.056 else 0.045,
           inflation = if (scenario == "historic") 0.028 else 0.025,  
...
emtr_nominal_historic_lower_assumptions() %>%
  select(-marginal.tax.rate) %>%  # duplicated
  setnames(names(.), gsub("[._]", " ", names(.))) %>% .[] %>%
  xtable(align = "lllrrrrrrr") %>%
  print.xtable(floating = FALSE, 
               booktabs = TRUE, 
               include.rownames = FALSE, 
               add.to.row = list(pos = list(which(.$`return type`[-1] != .$`return type`[-nrow(.)])), 
                                 command = "[4pt]"))
@
\end{table*}
...
grattan:::chart_data(bind_rows("NG" = data_frame(Taxable_Income = density_NG$x, 
...
\begin{table*}
\centering
\caption{Effective marginal tax rates}\label{tbl:Real-EMTR-appendix}
...
omit_repeated <- function(x){
  ifelse(x != lag(x) | is.na(lag(x)), x, NA_character_)
}

data.table::CJ(
  capital_gain_return = c(0.10, 0.073, 0.06, 0.04), 
  ...
  # cosmetics
  mutate(capital_gain_return = paste0("Nominal annual capital gain__", latex_percent(capital_gain_return, percentage_symbol = "\\%"))) %>%
  spread(capital_gain_return, emtr) %>% 
  ungroup %>%
  arrange(desc(marginal_tax_rate)) %>%
  group_by(marginal_tax_rate, return_type) %>%
  arrange(desc(`Nominal annual capital gain__10.0\\%`)) %>%
  ungroup %>%
  mutate(`CGT treatment` = ifelse(Indexed, "Indexed", paste(latex_percent(CGT_discount, digits = 0, percentage_symbol = "\\%"), "discount"))) %>%
  select(-Indexed, -CGT_discount) %>%
  mutate_each(funs(percent1_01_TeX)) %>%
  ...
  mutate(`\\phantom{.}` = ifelse(!is.na(`Marginal tax rate`) & `Marginal tax rate` != first(`Marginal tax rate`), 
                                 "[6pt]", 
                                 ifelse(!is.na(`Return type`) & `Return type` != first(`Return type`), 
                                        "[3pt]",
                                        ""))) %>%
  select(`\\phantom{.}`, 
         `Marginal tax rate`, `Return type`, `CGT treatment`,
         `Nominal annual capital gain__10.0\\%`, 
         `Nominal annual capital gain__7.3\\%`, 
         `Nominal annual capital gain__6.0\\%`, 
         `Nominal annual capital gain__4.0\\%`) %>%
  {suppressWarnings(print_2heading_xtable(., 
                                          sanitize.text.function = function(x) x, 
                                          tabular.environment = "tabular", 
                                          xtable.align = "lllllrrrr",
                                          floating = FALSE))}
