...
knit_hooks$set(timeit = function(before) {
    if (before) {
      ...
      if (exists("sample_file")) stopifnot(is.data.table(sample_file))
...
target_sample_file <- 
  ...
  selector(cols = keep.names)
rm(keep.names)
@

<<Import-HILDA, cache=TRUE>>=
hilda_w14_hh <- unique(as.data.table(hildaData::Combined_n150c, by = "nhhrhid"))
hilda_used <- TRUE
@
...
<<Import-SIH>>=
sih201314 <- read_dta("~/Data/ABS/IncomeHouse2013-14_Stata/Stata files/sih13bh.dta")
@
...
revenue_quarantine_NG <- function(sample_file = sample_file_1314,
...
revenue_quarantine_NG_2 <- function(sample_file = sample_file_1314,
                                    new_CGT_discount = new.discount,
                                    sample_file_fy_year = use.sample.fy,
                                    target_fy = TARGET.FY,
                                    age_based = TRUE,
  ...
  marginal_tax_rate_CG <- 
    sample_file_1314 %>%
    .[Net_CG_amt > 0] %>%
    .[, marginal_rate := income_tax(Taxable_Income + 1, "2013-14") - income_tax(Taxable_Income, "2013-14")] %$%
    mean(marginal_rate)
  ...
  target_sample_file %>%
    ...
    .[, post_tax_yearZ := income_tax(pmaxC(new_Taxable_Income_yearZ, 0), fy.year = "2015-16")] %>%
...
sample_file %>%
  ...
  .[, text.label := if_else(`Taxable income (less CG) percentile` == max(`Taxable income (less CG) percentile`),
    ...
    by = "Age group"] %>%
  # choice of 96 cosmetic (puts labels in a reasonable place semantically and aesthetically)
  .[, text.y := mean(probCG[`Taxable income (less CG) percentile` > 96 & `Taxable income (less CG) percentile` < 100]),
    by = "Age group"] %>%
         {
...
data.table(
  ...
  mutate(labels = if_else(n == 10, gsub(")", ")\n", 
...
hilda_all_waves_rp %>%
  ...
  mutate(Profit_loss = if_else(grepl("Loss", FY_rental_property_profit_or_loss, fixed = TRUE), 
                              "Loss", 
                              if_else(grepl("Profit", FY_rental_property_profit_or_loss, fixed = TRUE),
                                     "Profit", 
                                     if_else(grepl("Break.even", FY_rental_property_profit_or_loss), 
...
hilda_rp_who_negatively_gearing <-
  ...
  mutate(
    ...
    left_censored = if_else(isTRUE(Year == min(Year)), 
                           TRUE, 
                           if_else(!is.na(first(fill_up, order_by = Year)), 
                                  first(fill_up, order_by = Year) == "Loss", 
                                  FALSE)
                                  ), 
    right_censored = if_else(isTRUE(Year == max(Year)), 
                            TRUE, 
