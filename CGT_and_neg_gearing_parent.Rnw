...
<<downloadGrattex>>=
overwrite_grattex <- TRUE# && (!file.exists("grattan.cls") || (difftime(Sys.time(), file.info("grattan.cls")$ctime, units = "days") > 2))
...
A better balance between the competing considerations would reduce the discount so that \Sexpr{latex_percent(1 - new.discount)} of capital gains are taxed, which would raise around \Sexpr{texNum(cost_of(policy_reduce_CGT), dollar = TRUE)} a year.
