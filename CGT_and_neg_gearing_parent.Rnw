...
# Better ways of infering the distribution, but I'll let it slip! 
@
...
# Are there any checks for what the inflating factors are? Have John and Dani sense-checked these? Ideally I'd want to see a plot of historical price and projection for all the series you're using to inflate. 
@
...
# Again, not a huge sin but it's more standard to use geometric averages for TTY stocks
...
# Probably won't make a material difference, but it'd be more standard to do the calculations at high frequency then aggregate. 
interest_losses <-
  merge(total.investor.loans, rba_interest) %>%
  merge(., historical.interest.losses, all.x = TRUE) %>%
  mutate(interestXloans = mean.i * loans_amt) # So this is interest paid on investment loans

# Good enough 
#plot(interest_losses$tot.rent_interest_deds, interest_losses$interestXloans)
#summary(lm(tot.rent_interest_deds ~ interestXloans, data = interest_losses))
rental.losses <-
interest_losses[!complete.cases(interest_losses), ] %>%
  do({
    model <- lm(tot.rent_interest_deds ~ interestXloans, data = interest_losses) # Should it be data = . ? 
    pred <- predict(model, newdata = .[c("interestXloans")])
    data.frame(., pred)
  }) %>%
  # Why replace this with a linear estimate? 
...
# So the assumption here is that capital gains are unchanged. I think this is a HUGE assumption. 
#' Let's say a firm earns profits of 10% of its equity a year and it was started with $1000. 
# Now what happens if it retains the earnings (capital gain of $100 a year). Under this scenario, it pays $30 in corporate income tax
# and has a capital gain of $70 in cash + $30 in franking credits. The owner, on a MTR of 45% pays $45 in taxes up front (but has a claim on future franking credits). Under the old regime, they only paid $22.50 and had a claim on the $30 of franking credits. So the change in income for the owner has gone from $77.50 to $55 a year--- the rate of return has fallen by 2%!
# In asset pricing, the equilibrium asset price growth is equal to the earnings growth (Cochrane 1994). So my fairly strong view is that you need to reduce assumed asset price growth by the reduction in earnings in the medium run. Imagine what would be true if 
# this equilibrium relationship were not true! There would be a long-run time trend in PE ratios... 

@

<<revenue_abolish_NG>>=
revenue_abolish_NG <- function(fy.year = "2015-16"){
  sample_file_1314 %>%
    select(-contains("MCS")) %>%  # project not good
    project_to(fy.year, fy.year.of.sample.file = use.sample.fy) %>%
    # I'm pretty sure this is why they invented ifelse()...
...
cgt_discount_distribution <- 
  ...
  # I'm not sure I like this. If you're a trader with 0 profits, your Tot_CY_CG_amt is huge, but your Net_CG_amt is small. 
  # This explains a lot of the bulk at apparent_discount = 1
...
revenue_quarantine_NG_year0 <- function(sample_file = sample_file_1314, new_CGT_discount = new.discount, sample_file_fy_year = use.sample.fy, .old.discount = 0.50){
  ...
  target_sample_file %>%
    ...
    # What does this mean, given you have filtered  the above using filter(Tot_CY_CG_amt > 0). So you calculate the probability
    # of not getting a discount conditional on having some positive gross capital gains, and then 
    # apply this to the full dataset... 
    # Makes my puny brain hurt, but it doesn't feel right. If we hand out discounts to everyone, why would somebody have no (or less than 50%) discount? 
    ...
    mutate(
      # Why the rejection sampling? Please explain. 
...
# Up to here. Need to go to sleep. 
@
...
revenue_quarantine_NG <- function(sample_file = sample_file_1314, new_CGT_discount = new.discount, sample_file_fy_year = use.sample.fy, target_fy = TARGET.FY, age_based = TRUE, Z = 20){
  ...
  if (!age_based){
    ...
    q_by_integer_age <- 
      data.table(age_range = rep(q_by_age$age_range, each = 5)) %>% # This is not working. 
...
A better balance between the competing considerations would reduce the discount so that \highlight{\Sexpr{latex_percent(1 - new.discount)} of capital gains are taxed}, which would raise around \highlight{\Sexpr{texNum(cost_of(policy_reduce_CGT), 2L, dollar = TRUE)}} a year.
...
individuals_table1_201314 %>%
  ...
  scale_y_continuous(label = function(x){paste0(dollar(x / 1e9), "bn")}) + 
...
\begin{table}
<<Number-households-SIH, results='asis', eval = F>>=
sih201314 %>%
...
\begin{bigbox*}{Capital gains tax and asset lock-in}{box:CGT-asset-lock-in}
