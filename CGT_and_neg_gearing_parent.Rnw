    ...
    text.y = mean(avg_CGT[`Taxable income (less CG) percentile` > 94])
         ) %>%
         {
           grplot(., aes(x = `Taxable income (less CG) percentile`, y = avg_CGT, 
                         color = rev(`Age group`), weight = n_taxpayers)) + 
             theme(axis.title.y = element_blank(),
                   axis.title.x = element_text(hjust = 0.5, margin = margin(11, 0, 0, 0, unit = "pt"), face = "bold")) +  
             # gridlines go to far
...
data.table(
  ...
  mutate(labels = ifelse(n == 10, gsub(")", ")\n", 
                                       gsub("EMTR", "", as.character(CGT_regime)), 
                                       fixed = TRUE), NA_character_)) %>%
  rename(`Holding period for assets (years)` = n) %>%
  {
    grplot(., aes(x = `Holding period for assets (years)`, 
      ...
      geom_text_repel(aes(label = labels),
                      fontface = "bold", size = 22/(14/5), 
                      point.padding = unit(0.05, "npc"),
                      na.rm = TRUE) + 
      scale_y_continuous(label = percent, breaks = (0:6)/10, limits = c(min(.$EMTR) - 0.05, max(.$EMTR) + 0.05), expand = c(0,0)) + 
...
emtr_nominal_historic_lower_assumptions() %>% .[] %>%
  select(-marginal_tax_rate) %>%  # duplicated
  setnames(names(.), gsub("[._]", " ", names(.))) %>% .[] %>%
  mutate_each(funs(omit_repeated)) %>%
  print_2heading_xtable(separator = "XXX", xtable.align = "lllrrrrrrr", floating = FALSE) 
@
\end{table*}
...
density_NG <- 
  sample_file_1314 %>%
  select(Net_rent_amt, Taxable_Income) %>%
