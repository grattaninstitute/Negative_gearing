...
Quarantining losses so they can only be written off against other investment income (operating profits and capital gains) could raise around \Sexpr{texNum(cost_of(policy_year0), dollar = TRUE, sig.figs = 1L)} a year in the short-term. This would decline to around \Sexpr{texNum(cost_of(policy), dollar = TRUE, sig.figs = 1L)} over time as those losses are offset against investment income. 

An alternative reform would align the tax deductions for capital losses with that for gains.  For example, if the capital gains tax discount is reduced to \Sexpr{latex_percent(new.discount)} (investors pay tax on \Sexpr{latex_percent(1 - new.discount)} of their capital gains), then investors could deduct \Sexpr{latex_percent(1 - new.discount)} of their investment losses. Treating capital gains and recurrent losses consistently would reduce tax incentives to borrow to invest. It would contribute around \Sexpr{texNum(cost_of(policy_align_gains_and_losses), dollar = TRUE, sig.figs = 2L)} a year to the bottom line. 
