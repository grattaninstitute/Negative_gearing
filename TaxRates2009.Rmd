...
shinyApp( options = list(height = 1024),
  ...
  server = function(input, output) {
    sliderValues <- reactive({
      ...
      fread("./IndividualSampleFile/2009-10/Sample_file_0910/2010 1% individuals sample file/2010 1% individuals sample file.txt", select="Taxable_Income") %>% 
        mutate(diff = income_tax(Taxable_Income, fy.year="2009-10") - new_income_tax(Taxable_Income)) %$% 
        {-1 * sum(diff)*50} %>%
        grattan_dollar(.) ->
      ...
      data.frame(
        ...
        Value = as.character(c(input$rate_1, 
                               ...
                               input$rate_5,
                               the_amount_remaining,
                               grattan_dollar(new_income_tax(cpi_inflator_quarters(100e3, nominal.date = "2014-Q4", target.date = "2009-Q4"))))), 
    ...
    chartValues <- reactive({
      ...
      p <- data.table(
          ...
          mutate(
            marginal.tax = income_tax(Taxable_Income + 1, fy.year = "2009-10") - income_tax(Taxable_Income, fy.year = "2009-10"),
