...
Because capital gains are lumpy, some lower income earners will have relatively high taxable incomes in the year they realise gains. But even if we look at taxable incomes before capital gains, most gains are earned by those in the top 10 per cent of income earners and by those with little or no taxable income.%
\footnote{Almost \Sexpr{latex_percent(prop_inc_CG_top_decile_excl_CG)} of capital gains are earned by those in the decile of income earners \emph{excluding} capital gains and another \Sexpr{latex_percent(prop_inc_CG_bottom_decile_excl_CG)} -- that is, their other income is less than \TBD{\$X}.}
